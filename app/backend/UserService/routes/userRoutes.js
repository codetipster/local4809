...
// adding land to user's land list when a user(landowner) creates a land listing
router.post('/updateLandList', async (req, res) => {
    const { userId, landId } = req.body;
    try {
        // Logic to add landId to the user's Lands array
        const user = await User.findById(userId);
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }

        user.Lands.push(landId);
        await user.save();
        res.status(200).json({ message: 'Land list updated successfully' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
